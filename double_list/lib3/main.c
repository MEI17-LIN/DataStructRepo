...
int main()
{
    ...
    {   
        ...
        ret = handle->insert(handle, &stu, LLIST_FORWARD);
    ...
    {
        tmp = handle->find(handle, &stu_id, id_cmp);
        if(tmp == NULL)
    ...
    while(scanf("%d",&stu_id) == 1) 
    {
        status = handle->delete(handle, &stu_id, id_cmp);
        if(status != 0)
            printf("id %d can not be found.\n", stu_id);
        else
    ...
    while(scanf("%d",&stu_id) == 1) 
    {
        status = handle->fetch(handle, &stu_id, id_cmp, tmp);
        if(status != 0)
            printf("id %d can not be found.\n", stu_id);
        else
            {
